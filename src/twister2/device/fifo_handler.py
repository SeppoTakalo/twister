...
class FifoHandler:
    ...
    def __init__(self, fifo_in: str | Path, fifo_out: str | Path):
        """
        :param fifo: path to fifo file
        """
        self._fifo_in = fifo_in
        self._fifo_out = fifo_out
        self._dump_file_path = Path(fifo_in).parent / HANDLER_LOG_FILE_NAME
        self.file_in: io.BytesIO | None = None
        self.file_out: io.BytesIO | None = None
        self.dump_file: io.TextIOBase | None = None
        self._threads: list[threading.Thread] = []
        self._created_fifos = False

    def _make_fifo_file(self, filename: str) -> None:
        if os.path.exists(filename):
            logger.debug('Fifo exist: %s', filename)
            return
            #os.unlink(filename)
        os.mkfifo(filename)
        self._created_fifos = True
    ...
    def connect(self):
        ...
        logger.debug('Log out: %s', self._dump_file_path)
